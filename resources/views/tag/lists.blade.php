...
<div class="page-container">
	<div class="text-c">
		<form class="Huiform" method="get" action="{{URL::Route('tag.lists')}}" target="_self">
			<div class="text-c">
				<span class="select-box inline">
					<select name="type" class="select">
						<option value="">全部</option>
						<option value="hospital">医院标签</option>
						<option value="subject">部门标签</option>
					</select>
				</span>
	...
	<table class="table table-border table-bordered table-bg">
		...
		@foreach($lists as $val)
			<tr class="text-c">
				...
				<td>{{$val->type}}</td>
				<td>
					<a title="编辑" href="javascript:;" onclick="tag_edit('标签编辑','{{ URL::route('tag.detail',['id'=>$val->id]) }}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
